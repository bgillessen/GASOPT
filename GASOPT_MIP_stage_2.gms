...
$CALL gdxxrw.exe OutputBoundStrengthening_stage_2.xlsx Squeeze=N @read_sets.txt trace=3
$GDXIN OutputBoundStrengthening_stage_2.gdx
...
$CALL gdxxrw.exe OutputBoundStrengthening_stage_2.xlsx Squeeze=N @read_data.txt trace=3
$GDXIN OutputBoundStrengthening_stage_2.gdx
...
MIPSol_configuration(CSID,Configuration)
MIPSol_element(CSID,ElementID)
;

$onecho > read_station_settings.txt
par=MIPSol_configuration rng=CS_configuration!A1 rdim=2
par=MIPSol_element rng=CS_element!A1 rdim=2
$offecho

$CALL gdxxrw.exe GASOPT_MIP_stage_1_results.xlsx Squeeze=N @read_station_settings.txt trace=3
$GDXIN GASOPT_MIP_stage_1_results.gdx
$LOAD MIPSol_configuration MIPSol_element
$GDXIN

u_configuration.fx(CSID,Configuration)$CSConfigExist(CSID,Configuration) = MIPSol_configuration(CSID,Configuration);
u_element.fx(CSID,ElementID)$CSElementExist(CSID,ElementID) = MIPSol_element(CSID,ElementID);

display  MIPSol_configuration, MIPSol_element, Q.lo, Qsqr.lo, Q.up, Qsqr.up, u_configuration.l, u_element.l;
...
$onecho > write_GASOPT_MIP_stage_2_results.txt
...
execute_unload "GASOPT_MIP_stage_2_results.gdx";

execute 'gdxxrw.exe GASOPT_MIP_stage_2_results.gdx o=GASOPT_MIP_stage_2_results.xlsx Squeeze=N @write_GASOPT_MIP_stage_2_results.txt';
